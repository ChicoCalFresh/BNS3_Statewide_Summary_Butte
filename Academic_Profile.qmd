...
plot_frq(bns$q16, geom.colors = plot_color, ylim=c(0, ylim=c(0, 1.2*max(table(bns$q16)))), wrap.labels = 16, sort.frq = 'desc') + xlab('') + ylab("Number of Students") + theme(axis.text.x = element_text(size = 10))
...
plot_frq(bns$gpa_fac, geom.colors = plot_color) + xlab('GPA') + ylab("Number of Students") + ylim(0,1.2*max(table(bns$gpa_fac)))
```
