...
bns_UC <- bns_unfiltered %>% filter(bns_unfiltered$school == "UC Berkeley")

bns_CSU <- bns_unfiltered %>% filter(bns_unfiltered$school == "Cal State LA" | 
                  bns_unfiltered$school =="Cal State San Bernardino" |
                  bns_unfiltered$school == "CSU Bakersfield" | 
                  bns_unfiltered$school == "CSU Dominguez Hills" | 
                  bns_unfiltered$school == "Sacramento State" | 
                  bns_unfiltered$school == "San Francisco State")

bns_CCC <- bns_unfiltered %>% filter(bns_unfiltered$school == "Butte CC" | 
                  bns_unfiltered$school =="Allan Hancock Community College" |
                  bns_unfiltered$school == "Clovis CC" | 
                  bns_unfiltered$school == "Mt. SAC CC" | 
                  bns_unfiltered$school == "Palo Verde CC")
...
**Summary: ** The average age of the students was `r round(mean(bns$age, na.rm = TRUE),1)` years with a range of `r min(bns$age, na.rm = TRUE)` to `r max(bns$age, na.rm = TRUE)`, and a median age of `r median(bns$age)`. A total of `r get_count_and_percent3(bns$age, 21)` students were 21 or older, and `r get_count_and_percent3(bns$age, 30)` students were 30 or older. Mean ages differed between campus types, where the mean age among all CSU students was `r round(mean(bns_CSU$age, na.rm = TRUE),1)`, compared to `r round(mean(bns_CCC$age, na.rm = TRUE),1)` among all CCC students, and `r round(mean(bns_UC$age, na.rm = TRUE),1)` among all UC students. 
...
plot_frq(rec$q7, sort.frq="desc", geom.colors = plot_color, drop.empty = FALSE) + xlab('Gender') + 
  ylab("Number of Students") + ylim(0, 1.3*max(table(rec$q7)))
...
plot_frq(tmp$hispanic, geom.colors = plot_color, ylim=c(0, 1.3*max(table(tmp$hispanic)))) + xlab("") + ylab("Number of Students")
```
:::
...
**Summary: ** A total of `r get_count_and_percent(tmp$white, '1')` students self-identified as White, `r get_count_and_percent(tmp$amer_indian, '1')` as American Indian or Alaskan Native, `r get_count_and_percent(tmp$black, '1')` as Black or African American, `r get_count_and_percent(tmp$asian, '1')` as Asian,  and `r get_count_and_percent(tmp$hawaiian, '1')` as Native Hawaiian or Other Pacific Islander. Additionally, `r get_count_and_percent(tmp$eth_PNTA, '1')` students preferred not to answer race, likely due to confusion between race and ethnicity. 
...
p1$data$valid.prc <- signif(p1$data$valid.prc, digits = 1)
...
ggplot(tmp2, aes(x=factor(Var1, levels=Var1), y=Freq, fill=FirstGen, label=Percent)) + geom_col(colour = "black") +
  scale_fill_manual(values=c("#336699", "#00944d", "grey")) + geom_text(aes(y=Freq+4)) + ylim(0,1.3*max(table(bns$q13))) +
  scale_x_discrete(labels=label_wrap(10)) +   geom_text(aes(y=Freq + 8, label=Freq)) +
