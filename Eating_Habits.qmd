...
plot_frq(bns$q77, geom.colors = plot_color) + xlab('Daily Fruit & Vegetable Servings') + ylab("Number of Students") + ylim(0, 1.2*max(table(bns$q77)))
...
plot_frq(bns$q78, geom.colors = plot_color) + xlab('Daily Sugar-Sweetened Beverage Intake (Ounces)') + ylab("Number of Students") + ylim(0, 1.3*max(table(bns$q78)))
...
plot_frq(bns$q79, geom.colors = plot_color) + xlab('Weekly Fast Food Intake') + ylab("Number of Students")+ ylim(0, 1.2*max(table(bns$q79)))
```
...
plot_frq(bns$q80, geom.colors = plot_color, drop.empty = FALSE) + xlab('Weekly Alcohol Intake') + ylab("Number of Students") + ylim(0, 1.2*max(table(bns$q80)))
...
**Summary:** A total of `r count_and_percent(bns$q80, "Zero days")` reported drinking alcohol zero times per week, `r count_and_percent(bns$q80, "1-2 days")` reported drinking alcohol one to two days a week, and the remaining `r count_and_percent(bns$q80, "3-4 days", "5-6 days", "Everyday")` reported drinking alcohol three to four days per week. 
...
plot_frq(bns$q81, geom.colors = plot_color, drop.empty = FALSE) + xlab('Alcohol Servings Consumed When Drinking') + ylab("Number of Students") + ylim(0, 1.3*max(table(bns$q81)))
